## This script has been provided by Domenico Moramarco (U Bari)
## on request.
## It is used to generate within and between decomposition measures by 
## Bhattacharya and Mahalanobis [G_wit_add or GwA] 
## and Foster and Shneyerov [G_wit_path or GwP]
library(ineq)
library(dineq)
library(haven)
library(dplyr)
...
###########################################################################
###### This part is manually added by the autors  #########################
###### Function to calculate the squared CV       #########################
scv_decomp <- function(df) {
  x <- df$Rating
  g <- as.factor(df$Approval)
  
  # overall mean
  mu <- mean(x, na.rm = TRUE)
  
  # total SCV
  SCV_total <- var(x, na.rm = TRUE) / mu^2
  
  # group means and sizes
  group_means <- tapply(x, g, mean, na.rm = TRUE)
  group_sizes <- tapply(x, g, function(v) sum(!is.na(v)))
  
  # between SCV
  SS_between <- sum(group_sizes * (group_means - mu)^2)
  SCV_between <- SS_between / ((sum(group_sizes) - 1) * mu^2)
  
  # within SCV
  SS_within <- sum(tapply(x, g, function(v) sum((v - mean(v, na.rm = TRUE))^2, na.rm = TRUE)))
  SCV_within <- SS_within / ((sum(group_sizes) - length(group_means)) * mu^2)
  
  return(list(
    SCV_total   = SCV_total,
    SCV_within  = SCV_within,
    SCV_between = SCV_between,
    check       = SCV_within + SCV_between
  ))
}


#
#save.image("DATA/Moramarco.RData")
